...
def get_recsys_model(recsys_type: str = None, device=None):
...
def rec_sys_personalized(
    ...
    if model is None or isinstance(model, tuple):
        model = get_recsys_model(recsys_type="twitter", device=device)
    ...
    if len(post_ids) <= max_rec_post_len:
        ...
        for idx in range(1, len(rec_matrix)):
            ...
            for post_id, post_content in available_post_contents:
                if model is not None:
                    user_embedding = model.encode(user_bio, device=device)
                    post_embedding = model.encode(post_content, device=device)
            ...
            for _post_id, _base_similarity in post_scores:
                ...
                like_similarity = (
                    sum(
                        np.dot(
                            model.encode(_post_content, device=device),
                            model.encode(like, device=device),
                        )
                        / (
                            np.linalg.norm(model.encode(_post_content, device=device))
                            * np.linalg.norm(model.encode(like, device=device))
                        )
                ...
                dislike_similarity = (
                    sum(
                        np.dot(
                            model.encode(_post_content, device=device),
                            model.encode(dislike, device=device),
                        )
                        / (
                            np.linalg.norm(model.encode(_post_content, device=device))
                            * np.linalg.norm(model.encode(dislike, device=device))
                        )
